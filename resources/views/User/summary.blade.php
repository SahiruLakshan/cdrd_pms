...
<style>
    ...
    .progress-container {
        position: relative;
        width: 300px;
        /* Same as the width of the progress bar */
        height: 30px;
    ...
    progress {
        height: 30px;
        /* Adjust height to match the progress container */
    }
</style>
@section('content')
    <div class="text-center mt-3">
        <h4 style="font-family: inter;color:white;text-transform: uppercase;text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5);">
            {{ $wing_name }}</h4>
    </div>
    <div class="text-center mt-3">
        <h4 style="font-family: inter;color:white;text-transform: uppercase;text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5);">
            Overall Summary of Ongoing Projects</h4>
    </div>
    <table class="table mt-3" style="border: 2px solid white;margin: auto;width:1300px">
        ...
        <tbody>
            ...
            @foreach ($projects as $project)
                ...
                <tr>
                    <td
                        style="color: rgb(255, 255, 255); border-right: 2px solid white;font-weight: 700;width:100px;white-space: normal;text-align:justify">
                        {{ $x }}</td>
                    <td
                        style="color: rgb(255, 255, 255); border-right: 2px solid white;font-weight: 700;width:100px;white-space: normal;text-align:justify">
                        Project {{ $project->no }} <br>RC{{ $project->rc }}
                        <br>({{ \Carbon\Carbon::parse($project->start_date)->format('Y') }})
                    </td>
                    <td
                        style="color: rgb(255, 255, 255); border-right: 2px solid white;font-weight: 700;width:1200px;white-space: normal;text-align:justify">
                        {{ $project->pname }}<br><br>
                    ...
                    <td style="width:800px">
                        <div class="progress-container">
                            <span style="color:white"><b>Work Progress:</b></span>&nbsp;&nbsp;&nbsp;
                            <progress value="{{ $project->tasks_sum }}" max="100"></progress>
                            <span class="progress-text"
                                style="font-size: 12px">{{ number_format($project->tasks_sum, 1) }}% Completed</span>
                        </div><br>

                        <div class="progress-container">
                            <span style="color:white"><b>Time
                                    Line:</b></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            <progress value="{{ round($project->completed_months) }}"
                                max="{{ round($project->total_months) }}"></progress>

                            @if (Carbon::parse($project->end_date)->lessThan(Carbon::today()))
                                <span class="progress-text" style="font-size: 12px">Time Completed</span>
                            @else
                                <span class="progress-text" style="font-size: 12px">{{ round($project->completed_months) }}
                                    Months Completed</span>
                            @endif
                            ...
                            <div class="row">
                                ...
                                <div class="col mt-1">
                                    <div style="width: 10px; height: 10px; background-color: white;margin-left:100px ">
                                    </div>
                                </div>
                                <div class="col">
                                    @if (Carbon::parse($project->end_date)->lessThan(Carbon::today()))
                                    ...
                                    @else
                                        <span style="color:white;font-size: 12px">{{ round($project->remaining_months) }}
                                            Months Remaining</span>
                                    @endif
                                </div>
                            </div>
                        </div><br><br>
